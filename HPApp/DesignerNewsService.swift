...
//    private static let baseURL = "http://hacpai.com"
    ...
    private enum ResourcePath: Printable {
        ...
        var description: String {
            ...
            case .Checkin: return "/api/v1/activities/checkin"
    ...
    static func upvoteStoryWithId(storyId: Int, token: String, response: (successful: Bool) -> ()) {
        ...
        postWithToken(resourcePath, token: token, response: response)
    }
    ...
    static func checkin(token: String, response: (checkin: Checkin?, error: Error?) -> ()) {
        let resourcePath = ResourcePath.Checkin
        checkinWithResourcePath(resourcePath, token: token, response: response)
    }

    // MARK: Private Methods
    
    private static func checkinWithResourcePath(path: ResourcePath, token: String, response: (checkin: Checkin?, error: Error?) -> ()) {
        checkinWithToken(path, token: token, response: response)
    }
    
    private static func checkinWithToken(path: ResourcePath, token: String, response: (checkin: Checkin?, error: Error?) -> ()) {
        ...
        Alamofire.request(request).responseJSON { (_, urlResponse, json, error) in
            if let checkinDict = json?["checkin"] as? NSDictionary {
                let checkin = JSONParser.parseCheckin(checkinDict)
                response(checkin: checkin, error: nil)
            }
        }
    }

    private static func postWithToken(path: ResourcePath, token: String, response: (successful: Bool) -> ()) {
        let urlString = baseURL + path.description
        let request = NSMutableURLRequest(URL: NSURL(string: urlString)!)
        request.HTTPMethod = "POST"
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
