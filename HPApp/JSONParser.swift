...
struct JSONParser {
    ...
    static func parseCheckin(checkin: NSDictionary) -> Checkin {
        let balance = checkin["balance"] as? Int ?? 0
        let reward = checkin["reward"] as? Int ?? 0
        let streak = checkin["streak"] as? Int ?? 0
        let streaked = checkin["streaked"] as? Int ?? 0
        
        return Checkin(balance: balance, reward: reward, streak: streak, streaked: streaked)
    }
